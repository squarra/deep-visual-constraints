...
def get_camera_projection_matrix(cam_distance, r=1.0):
...
def render_mesh_rai2(
    ...
    for i, (phi, theta) in enumerate(zip(phi, theta)):
        ...
        C.addFrame(cam_name, "mug").setShape(ry.ST.marker, [0.001]).setRelativeAffineMatrix(camera_transform)

        random_pos = np.array([0, 0, 0.9]) + np.random.randn(3) * np.array([0.5, 0.5, 0.1])
...
def render_mesh_rai(
    ...
    for i, (phi, theta) in enumerate(zip(phi, theta)):
        ...
        cam_name = "camera" + str(i)
        C.addFrame(cam_name, "mug").setShape(ry.ST.marker, [0.0]).setRelativeAffineMatrix(camera_transform)
        random_pos = np.array([0, 0, 1.9]) + np.random.randn(3) * np.array([0.5, 0.5, 0.1])
        random_quat = np.random.randn(4)
        random_quat /= np.linalg.norm(random_quat)
        mug.setQuaternion(random_quat).setPosition(random_pos)  # randomizes lighting
...
def render_mesh(mesh_filename, num_cam=10, cam_distance_center=15, sig_cam_distance=3.0, obj_radius=1.0, obj_center=np.zeros(3), res=100, view=False):
...
def compute_sdf(obj_trimesh, N=5000, sig=0.01, scale=1.0, center=np.zeros(3), view=False):
    ...
    points = np.concatenate(
        [surface_points + np.random.randn(N, 3) * sig * scale, surface_points + np.random.randn(N, 3) * sig * scale * 10, global_samples], axis=0
    )
    ...
    if view:
        ...
        pyrender.viewer.Viewer(scene, use_raymond_lighting=True, point_size=10.0)

    return points, sdf


def compute_sdf2(obj_trimesh, N=5000, sig=0.01, scale=1.0, center=np.zeros(3), view=False):
    ...
    points = np.concatenate([surface_points + np.random.randn(N, 3) * sig, surface_points + np.random.randn(N, 3) * sig * 10, global_samples], axis=0)

    sdf = point_cloud.get_sdf_in_batches(points, use_depth_buffer=True, sample_count=10000000)
