...
from dvc.functional_object import FunctionalObject
...
class Frame(nn.Module):
    ...
    def build_keypoint_head(self, name, width, key_points, sdf_object=False, train_pts=False):
        ...
        layer_list = [nn.Linear(in_dim, width[0]), nn.ReLU(inplace=True)]
        for i in range(len(width) - 1):
            layer_list.extend([nn.Linear(width[i], width[i + 1]), nn.ReLU(inplace=True)])
        layer_list.append(nn.Linear(width[-1], 1))
        ...
        head.key_points = nn.parameter.Parameter(key_points.view(1, 1, num_points, 3), requires_grad=train_pts)
        head.name = name

    def extract_mesh(
        self, images=None, projection_matrices=None, center=[0, 0, 0], scale=0.15, num_grid=50, sdf_scale=10.0, delta=0.0, draw=True, return_sdf=False
    ):
