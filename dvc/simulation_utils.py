...
from dvc.feature import JIT_Collision_Feature, JIT_ICP_Feature, JIT_Keypoint_Feature, KeyPoint_Feature  # , JIT_PoseEstimation_Feature
...
class Simulation:
    def __init__(self, worldFileName, mug_inds=[], mug_poses=[], verbose=1, addBalls=False, sort=False, addImp=False):
        ...
        if addImp:
            self.S.addImp(ry.ImpType.noPenetrations, [], [])
        self.stepNone(3.0, False)
        if addBalls:
            MugPos = self.RealWorld.frame("mug" + str(mug_inds[0])).getPosition()
            for i in range(10):
                pos = MugPos + np.array([0, 0, 0.02 + 0.032 * i])
                b = self.RealWorld.addFrame("ball_" + str(i)).setShape(ry.ST.sphere, [0.015])
                b.setPosition(pos).setMass(0.00001).setColor([0.8, 0.6, 0.6])
            self.S = self.RealWorld.simulation(ry.SimulatorEngine.bullet, verbose)
            if addImp:
                self.S.addImp(ry.ImpType.noPenetrations, [], [])
            self.stepNone(3.0, False)
    ...
    def addMug(self, ind, pose):
        ...
        mug = self.RealWorld.addMeshFrame(mesh_coll_name, "mug" + str(ind), mass=mass, com=com)
        mug.setPosition(pose[:3]).setQuaternion(pose[3:])

    def closeGripper(self, ind, gripper_prefix=""):
        self.S.closeGripper(gripper_prefix + "gripper", speed=1.0, objFrameName="mug" + str(ind))
        while True:
            self.stepNone()
    ...
    def openGripper(self, gripper_prefix=""):
        self.S.openGripper(gripper_prefix + "gripper", speed=3.0)

    def goingBack(self, tau=3.0):
        self.stepPosition(self.qInit, tau)

    def showToCameras(self, tau=1.0):
        gripperTarget = self.RealWorld.frame("gripperCenter").getPosition()
        gripperTarget += np.array([0, 0, 0.2])
        ...
        komo.addObjective([], ry.FS.scalarProductXZ, ["world", "gripperCenter"], ry.OT.eq, target=[-1.0])
        komo.optimize()
    ...
    def stepNone(self, tau=None, realTime=True):
        ...
        for _ in range(int(tau / self.tau)):
            self.S.step([], self.tau, ry.ControlMode.none)
            if realTime and self.verbose > 0:
                time.sleep(self.tau)
    ...
    def takePicture(self, inds, draw=True):
        out = get_all_images(self.RealWorld, self.camera, self.camera_name_list, ["mug" + str(ind) for ind in inds], r=0.15, res=128)
...
class Configuration:
    def __init__(self, worldFileName, exp_name, mug_inds, view=True):
        ...
        state = torch.load("network/" + exp_name + ".pth.tar", map_location=torch.device("cpu") if self.device == "cpu" else None)
        config = state["config"]
        ...
        self.gripper_colls = ["gripper_coll", "L_finger_coll1", "L_finger_coll2", "R_finger_coll1", "R_finger_coll2"]
    ...
    def extractKeyFeatures(self, rgb_list, projection_list, obj_pos_list, obj_r_list):
        dx = torch.linspace(-1.0, 1.0, 10).to(self.device)
        ...
        for rgb, projection, obj_pos, obj_r in zip(rgb_list, projection_list, obj_pos_list, obj_r_list):
            rgb_tensor = torch.Tensor(rgb).permute(0, 3, 1, 2).to(self.device) / 255.0
            projection_tensor = torch.Tensor(projection).to(self.device)

            key_points = torch.stack([grid_x.flatten(), grid_y.flatten(), grid_z.flatten()], dim=1)
            key_points *= obj_r
            key_points += torch.Tensor(obj_pos).view(1, 3).to(self.device)

            with torch.no_grad():
                features = self.obj.backbone(key_points.unsqueeze(0), rgb_tensor.unsqueeze(0), projection_tensor.unsqueeze(0)).mean(dim=1)
    ...
    def updatePIFO(self, inds, rgb_list, projection_list, obj_pos_list, obj_r_list, compute_init_guess=True, target_list=None):
        """
        Args
        ...
        for i, (ind, rgb, projection, obj_pos, obj_r) in enumerate(zip(inds, rgb_list, projection_list, obj_pos_list, obj_r_list)):
            rgb_tensor = torch.Tensor(np.array(rgb)).permute(0, 3, 1, 2).to(self.device) / 255.0
            ...
            if target_list is not None:
                dx = torch.linspace(-1.0, 1.0, 5).to(self.device)
                grid_x, grid_y, grid_z = torch.meshgrid(dx, dx, dx)

                scene_points = torch.stack([grid_x.flatten(), grid_y.flatten(), grid_z.flatten()], dim=1) * 0.7
                scene_points *= obj_r
                scene_points += torch.Tensor(obj_pos).view(1, 3).to(self.device)

                for j, target_name in enumerate(target_list[i]["target_name_list"]):
                    JIT_ICP_Feature(self.obj, scene_points, target_list[i]["key_points_list"][j], target_list[i]["features_list"][j]).save(
                        "jit/ICPNet" + str(ind) + target_name + ".pt"
                    )

            vertices, faces, normals = self.obj.extract_mesh(center=obj_pos, scale=obj_r, delta=0.0, draw=False)
            ...
            if compute_init_guess:
                N_init = 10
                pos_init = obj_pos + np.array([0, 0, 0.2]) + np.random.randn(N_init, 3) * 0.05
                ...
                pos_init = obj_pos + np.random.randn(N_init, 3) * 0.1
    ...
    def solveKOMO(self, action_list, initSymbols=None, stepsPerPhase=10, verbose=3, animate=False):
        ...
        komo = self.C.komo(len(action_list), stepsPerPhase, 5.0, 2, False)
        ...
        if initSymbols is not None:
            for s in initSymbols:
                if s[0] == "grasp":
                    Sk.extend([[0.0, 0.0], ry.SY.stable, [s[1] + "gripper", "obj" + str(s[2])]])

        for k, action in enumerate(action_list):
            if action[0] == "grasp":
                Sk.extend([[k + 1.0, k + 1.0], ry.SY.stable, [action[1] + "gripper", "obj" + str(action[2])]])
            elif action[0] == "hang":
                Sk.extend([[k + 1.0, len(action_list)], ry.SY.stable, [action[1] + "hook", "obj" + str(action[2])]])
            elif action[0] == "pose":
                Sk.extend([[k + 1.0, len(action_list)], ry.SY.stable, ["gripper", "obj" + str(action[2])]])
        ...
        for k, action in enumerate(action_list):
            ...
            if action[0] == "grasp":
                komo.add_PFAccumulatedCollision(
                    [k + 0.7, k + 1.0], [meshName] + [action[1] + c for c in self.gripper_colls], "jit/sdfNet" + str(mug_ind) + ".pt", ry.OT.eq, [1e1]
                )
                komo.add_PFKeypointObjective(
                    [k + 1], [meshName, action[1] + "gripperCenter"], "jit/graspNet" + str(mug_ind) + ".pt", ry.OT.eq, [1e-1]
                )
                ...
                komo.addObjective(
                    [k + 0.7, k + 1.0],
                    ry.FS.positionRel,
                    [objName, action[1] + "gripperCenter"],
                    ry.OT.eq,
                    [1e1],
                    target=[0, 0, -1 / stepsPerPhase],
                    order=2,
                )

                poseInit = self.graspInitDict[str(mug_ind)]
                if poseInit is not None:
                    komo.addObjective([k + 1.0], ry.FS.poseRel, [action[1] + "gripperCenter", meshName], ry.OT.eq, [1e-1], target=poseInit)
                ...
                if len(action[1]) > 0:
                    for c1 in self.gripper_colls:
                        for c2 in self.gripper_colls:
                            komo.addObjective([k + 0.7, k + 1.0], ry.FS.distance, ["R_" + c1, "L_" + c2], ry.OT.ineq, [1e1])

            elif action[0] == "hang":
                komo.add_PFAccumulatedCollision(
                    [k + 0.7, k + 1.0], [meshName, action[1] + "hook_coll"], "jit/sdfNet" + str(mug_ind) + ".pt", ry.OT.eq, [1e0], margin=0.0
                )
                komo.add_PFKeypointObjective([k + 1.0], [meshName, action[1] + "hook"], "jit/hangNet" + str(mug_ind) + ".pt", ry.OT.eq, [1e0])
                komo.addObjective(
                    [k + 0.7, k + 1.0], ry.FS.positionRel, [objName, action[1] + "hook"], ry.OT.eq, [1e1], target=[0, 0, 1 / stepsPerPhase], order=2
                )

                poseInit = self.hangInitDict[str(mug_ind)]
                if poseInit is not None:
                    komo.addObjective([k + 1.0], ry.FS.poseRel, [action[1] + "hook", meshName], ry.OT.eq, [1e-1], target=poseInit)
            #                     komo.addObjective([k+1.], ry.FS.positionRel,
            ...
            #                                       [1e-1], target=poseInit[3:])
                ...
                komo.add_PFICPObjective([k + 1], meshName, "jit/ICPNet" + str(mug_ind) + action[1] + ".pt", ry.OT.eq, [1e-1])
...
def mask_in_sphere(x, mask_list, T_list, K_list):
    ...
    for mask, T1, K1_rai in zip(mask_list, T_list, K_list):
        ...
        uv_distance = 1.0 - np.linalg.norm(uv2, axis=0)
...
def find_obj_pos_size(mask_list, T_list, K_list):
    con1 = {"type": "ineq", "fun": lambda x: mask_in_sphere(x, mask_list, T_list, K_list)}
    con2 = {"type": "ineq", "fun": lambda x: x[3] - 0.01}
    fun = lambda x: x[3] ** 2
...
def warp_with_homography(rgb, H, res):
    ...
    rgb_focused = torch.nn.functional.grid_sample(rgb, grid, mode="bilinear", padding_mode="zeros", align_corners=True)  # (1, 3, res, res)
