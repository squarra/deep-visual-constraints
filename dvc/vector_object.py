...
from dvc.dataset import UnNormalize
from dvc.evaluation import check_grasp_feasibility, check_hang_feasibility
from dvc.utils import axis_angle_to_quaternion, quaternion_apply, quaternion_multiply
...
class vectorObject(nn.Module):
    ...
    def build_modules(self):
        ...
        self.cam_encoder = nn.Sequential(nn.Linear(4, self.C["FEAT_CAM"]), nn.ReLU(inplace=True))

        lifter_layers = [nn.Linear(self.C["FEAT_IMG"] + self.C["FEAT_CAM"], self.C["WIDTH_LIFTER"][0]), nn.ReLU(inplace=True)]
        for i in range(len(self.C["WIDTH_LIFTER"]) - 1):
            lifter_layers.extend([nn.Linear(self.C["WIDTH_LIFTER"][i], self.C["WIDTH_LIFTER"][i + 1]), nn.ReLU(inplace=True)])
...
class Frame_vec(nn.Module):
    ...
    def build_sdf_head(self, width):
        ...
        for i in range(len(width) - 1):
            layer_list.extend([nn.Linear(width[i], width[i + 1]), nn.ReLU(inplace=True)])
        layer_list.append(nn.Linear(width[-1], 1))
        self.sdf_head = nn.Sequential(*layer_list)

    def build_pose_head(self, name, width):
        layer_list = [nn.Linear(7 + self.backbone.out_dim, width[0]), nn.ReLU(inplace=True)]
        for i in range(len(width) - 1):
            layer_list.extend([nn.Linear(width[i], width[i + 1]), nn.ReLU(inplace=True)])
        layer_list.append(nn.Linear(width[-1], 1))
    ...
    def extract_mesh(
        self, images=None, cam_params=None, center=[0, 0, 0], scale=0.15, num_grid=50, sdf_scale=10.0, delta=0.0, draw=True, return_sdf=False
    ):
        ...
        if draw:
            mesh = Poly3DCollection(vertices[faces], facecolors="w", edgecolors="k", linewidths=1, alpha=0.5)
    ...
    def forward(self, points, images=None, cam_params=None, grad_method=None):
        ...
        if grad_method is None:
            ...
            with torch.enable_grad():
                ...
                grads = torch.autograd.grad(outputs=sdf, inputs=points, grad_outputs=torch.ones_like(sdf))[0]
                return sdf, grads
...
class Pose_Feature_vec(nn.Module):
    def __init__(self, frame, name):
        ...
        if name == "grasp":
            collision_shapes.append(
                {"shape": "capsule", "size": [0.15, 0.03], "pos": torch.Tensor([0, 0, 0.064]), "quat": torch.Tensor([-0.5, 0.5, -0.5, 0.5])}
            )
            fing_colls = torch.Tensor([[-0.058, 0, 0], [0.058, 0, 0], [-0.07, 0, 0.0256], [0.07, 0, 0.0256]])
            for pos in fing_colls:
                collision_shapes.append({"shape": "sphere", "size": [0.02], "pos": pos})

        elif name == "hang":
            collision_shapes.append({"shape": "capsule", "size": [0.15 * 2, 0.002], "pos": torch.zeros(3), "quat": torch.Tensor([1, 0, 0, 0])})
    ...
    def forward(self, poses, images=None, cam_params=None, grad_method=None):
        ...
        if grad_method is None:
            ...
            with torch.enable_grad():
                ...
                delta_q = torch.cat([torch.ones(B, N, 1, device=device), 0.5 * delta_x[..., 3:]], dim=2)
                ...
                grads = torch.autograd.grad(outputs=y, inputs=delta_x, grad_outputs=torch.ones_like(y))[0]
            return y, grads
    ...
    def eval_features(self, poses, w_coll=0.0, coll_margin=0.0, return_grad=False):
        """
        poses: (B,N,7)
        """
        ...
        if w_coll == 0.0:
            ...
            with torch.enable_grad():
                ...
                phi_coll = y_coll.clamp_(max=0.0).view(B, N, K, 1).sum(dim=2, keepdim=True)  # (B,N,1,1)

                if return_grad:
                    grads = torch.autograd.grad(outputs=phi_coll, inputs=delta_x, grad_outputs=torch.ones_like(phi_coll))[0]
    ...
    def optimize(
        ...
        with torch.no_grad():
            ...
            for i in range(max_iter):
                phi, J_phi = self.eval_features(poses, w_coll, coll_margin, return_grad=True)
                # (B,N,2,1), (B,N,2,6)
                ...
                alpha = 1.0 * torch.ones(B, N, 1).to(device)
                ...
                if i % print_interval == 0:
                    print("iter: {}, cost: {}, coll: {}, max_diff: {}".format(i, costs.max().item(), colls.max().item(), max_diff))
    ...
    def check_feasibility(self, poses, filenames, masses, coms):
        ...
        for b in range(B):
            try:
                ...
                if self.name == "grasp":
                    feasibility[b] = check_grasp_feasibility(pose.cpu().numpy(), mesh_coll_filename, mass, com)
                elif self.name == "hang":
                    feasibility[b] = check_hang_feasibility(pose.cpu().numpy(), mesh_coll_filename, mass, com)
            except:
